    ...
    >
      {options.map((option, index) => (
        <button
          ...
          } border border-white px-4 py-1 text-md font-medium hover:bg-white hover:border-white hover:text-black   transition duration-300 ease-in-out ${index === 0 ? 'rounded-l-md' : ''} ${index === options.length - 1 ? 'rounded-r-md' : ''} `}
        >
          {option}
        </button>
      ))}
